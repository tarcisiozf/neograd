...
func main() {
	...
	Y_train := Y[:1000]
	X_test := matrix.FromSlice(X[1000:]).Transpose().Divf(255)
	Y_test := Y[1000:]

	w1, b1, w2, b2 := gradientDescent(X_train, Y_train, 5000, 0.1)
	_, _, _, a2 := forwardPass(w1, b1, w2, b2, X_test)
	fmt.Println("Accuracy", accuracy(prediction(a2), Y_test))
