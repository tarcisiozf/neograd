...
func main() {
	...
	vocabSize := 276 // the desired final vocabulary size
	numMerges := vocabSize - 256
	ids := make([]int, len(tokens)) // copy so we don't destroy the original list
	copy(ids, tokens)
	...
	for i := 0; i < numMerges; i++ {
		...
		idx := 256 + i
		fmt.Println(pair, idx)
		ids = mergeToken(ids, pair, idx)
		merges[pair] = idx
	}

	fmt.Println(len(tokens))
	fmt.Println(len(ids))
	fmt.Println(float32(len(tokens)) / float32(len(ids)))

	vocab := make(map[int][]int)
	for i := 0; i < 256; i++ {
		vocab[i] = []int{i}
	}
...
func mergeToken(tokens []int, pair [2]int, idx int) []int {
	var out []int
	i := 0
	for i < len(tokens) {
